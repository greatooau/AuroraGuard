...
namespace AuroraGuard.DataAccess.Dapper;
...
public static class Snapshotter
{
    ...
    public static Snapshot<T> Start<T>(T obj) => new(obj);
}
...
public class Snapshot<T>(T original)
{
    private static Func<T, T>? _cloner;
    private static Func<T, T, List<Change>>? _differ;
    private readonly T _memberWiseClone = Clone(original);
    private readonly T _trackedObject = original;
    ...
    {
        return EqualityComparer<TU>.Default.Equals(first, default) &&
               EqualityComparer<TU>.Default.Equals(second, default) ||
               !EqualityComparer<TU>.Default.Equals(first, default) && first!.Equals(second);
    }

    private static Func<T, T, List<Change>> GenerateDiffer()
    {
        var dm = new DynamicMethod("DoDiff", typeof(List<Change>), [typeof(T), typeof(T)], true);
